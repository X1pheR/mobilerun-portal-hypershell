...
CANONICAL_REPO = Path(os.environ.get("HYPERSHELL_RELEASE_CANONICAL_REPO", "/__release_unconfigured__/repo"))
ALLOWED_RELEASE_WORKTREE_ROOT = Path(os.environ.get("HYPERSHELL_RELEASE_WORKTREE_ROOT", "/__release_unconfigured__/worktrees"))
STAGE_ROOT = Path(os.environ.get("HYPERSHELL_RELEASE_STAGE_ROOT", "/__release_unconfigured__/stage"))
BSM_PROFILES = Path("/run/config/bitwarden-profiles.json")
BSM_TOKEN = Path("/run/secrets/bitwarden-token")
BSM_PROFILE = os.environ.get("HYPERSHELL_RELEASE_BSM_PROFILE", "")
BSM_PROJECT_ID = os.environ.get("HYPERSHELL_RELEASE_BSM_PROJECT_ID", "")
KEYSTORE_SECRET_ID = os.environ.get("HYPERSHELL_RELEASE_KEYSTORE_SECRET_ID", "")
PASSWORD_SECRET_ID = os.environ.get("HYPERSHELL_RELEASE_PASSWORD_SECRET_ID", "")
GRADLE_CACHE = Path("/gradle-cache")
...
def _configured_identity(name: str) -> int:
    raw = os.environ.get(name, "")
    if not raw.isdigit() or int(raw) <= 0:
        return -1
    return int(raw)

BUILD_UID = _configured_identity("HYPERSHELL_RELEASE_BUILD_UID")
BUILD_GID = _configured_identity("HYPERSHELL_RELEASE_BUILD_GID")
...
def _single_profile_settings_file() -> tempfile.NamedTemporaryFile:
    ...
    if not BSM_PROFILE or re.fullmatch(r"[A-Za-z0-9._-]+", BSM_PROFILE) is None:
        raise ReleaseError("Bitwarden release profile selector is missing or invalid")
    profile = profiles.get(BSM_PROFILE) if isinstance(profiles, dict) else None
    if not isinstance(profile, dict):
        raise ReleaseError("configured Bitwarden release profile is missing")
    # Preserve the selected non-secret profile definition, but replace its host
    # token path with the fixed container-local secret mount. No unrelated Machine
    # Account token file is mounted into this executor.
    profile = dict(profile)
    profile["access_token_file"] = str(BSM_TOKEN)
...
def resolve_signing_material() -> tuple[bytes, str]:
    ...
    try:
        try:
            ...
            for label, value in (
                ("project", BSM_PROJECT_ID),
                ("keystore secret", KEYSTORE_SECRET_ID),
                ("password secret", PASSWORD_SECRET_ID),
            ):
                try:
                    UUID(value)
                except (ValueError, TypeError) as exc:
                    raise ReleaseError(f"Bitwarden {label} selector is missing or invalid") from exc
            provider.assert_project_id(BSM_PROJECT_ID)
            keystore_b64 = provider.get_secret_value(KEYSTORE_SECRET_ID, BSM_PROJECT_ID)
            password = provider.get_secret_value(PASSWORD_SECRET_ID, BSM_PROJECT_ID)
        except ReleaseError:
            raise
