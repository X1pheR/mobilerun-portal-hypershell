...
class ReverseConnectionService : Service() {
    ...
    companion object {
        ...
        const val ACTION_HYPERSHELL_START_SESSION = "eu.hypershell.mobile.START_SESSION"
    ...
    @Volatile
    private var onDemandSessionActive = false
    private var onDemandReconnectAttempt = 0
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        ensureForeground()
        if (intent?.action == ACTION_HYPERSHELL_START_SESSION) {
            if (onDemandSessionActive && isServiceRunning.get()) {
                Log.i(TAG, "onStartCommand: On-demand session already active; preserving current budget")
                return START_NOT_STICKY
            }
            Log.i(TAG, "onStartCommand: Starting bounded Hypershell on-demand session")
            try {
                KeepAliveController.disable(this)
            } catch (e: Exception) {
                Log.w(TAG, "Failed to clear stale keep-awake state before on-demand start", e)
            }
            onDemandSessionActive = true
            onDemandReconnectAttempt = 0
    ...
    override fun onDestroy() {
        super.onDestroy()
        if (onDemandSessionActive) {
            onDemandSessionActive = false
            try {
                KeepAliveController.disable(this)
            } catch (e: Exception) {
                Log.w(TAG, "Failed to disable keep-awake during on-demand destroy", e)
            }
        ...
        try {
            ...
            webSocketClient = object : WebSocketClient(uri, headers) {
                ...
                override fun onClose(code: Int, reason: String?, remote: Boolean) {
                    ...
                    if (HypershellOnDemandPolicy.shouldStopAfterClose(onDemandSessionActive, code)) {
                        finishOnDemandSession("server_close")
                        return
                    }
                    ...
                    when (val decision = retryController.onClose(reason, ::cancelPendingReconnects)) {
                        is ReverseConnectionRetryDecision.Stop -> {
                            ...
                            if (onDemandSessionActive) {
                                finishOnDemandSession("terminal_${state.name.lowercase()}")
                            } else {
                                handleWsDisconnected()
                            }
                            return
    ...
    private fun scheduleReconnect(connectionGeneration: Long = webSocketGeneration.current()) {
        ...
        val shouldGiveUp = if (onDemandSessionActive) {
            HypershellOnDemandPolicy.shouldGiveUp(reconnectStartedAtMs, now)
        } else {
            shouldGiveUpReconnecting(reconnectStartedAtMs, now)
        }
        if (shouldGiveUp) {
            Log.w(TAG, "Reconnect budget exhausted, giving up")
            isReconnecting.set(false)
            reconnectStartedAtMs = 0L
            ConnectionStateManager.setState(ConnectionState.DISCONNECTED)
            if (onDemandSessionActive) {
                finishOnDemandSession("reconnect_exhausted")
            } else {
                handleWsDisconnected()
            }
            return
        ...
        val reconnectDelayMs = if (onDemandSessionActive) {
            HypershellOnDemandPolicy.reconnectDelayMs(onDemandReconnectAttempt++)
        } else {
            RECONNECT_DELAY_MS
        }
    ...
    private fun disconnectByUser() {
        if (onDemandSessionActive) {
            finishOnDemandSession("user_disconnect")
            return
        }
    ...
    private fun finishOnDemandSession(reason: String) {
        if (!onDemandSessionActive) return
        Log.i(TAG, "Finishing Hypershell on-demand session: $reason")
        onDemandSessionActive = false
        onDemandReconnectAttempt = 0
        isServiceRunning.set(false)
        cancelPendingReconnects()
        try {
            ...
            Log.w(TAG, "Failed to disable keep-awake during on-demand cleanup", e)
        }
        handleWsDisconnected()
        closeWebSocket()
        ConnectionStateManager.setState(ConnectionState.DISCONNECTED)
